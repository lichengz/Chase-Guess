...
namespace FusionExamples.Tanknarok
{
    ...
    public class Player : NetworkBehaviour, ICanTakeDamage
    {
        ...
        [Networked] public BattleID battleID { get; set; }
        ...
        [Networked] public byte score { get; set; }
        ...
        private void SetMeshOrientation()
        {
            ...
            switch (_driveDirection)
            {
                case DriveDirection.FORWARD:
                    if (moveDirection.magnitude > 0.1f &&
                        Vector3.Dot(_lastMoveDirection, moveDirection.normalized) < 0f)
            ...
            if (moveDirection.magnitude > 0.1f)
                _hull.forward = Vector3.Lerp(_hull.forward,
                    new Vector3(moveDirection.x, 0, moveDirection.y) * multiplier, Time.deltaTime * 10f);

            if (aimDirection.sqrMagnitude > 0)
                _turret.forward = Vector3.Lerp(_turret.forward, new Vector3(aimDirection.x, 0, aimDirection.y),
                    Time.deltaTime * 100f);
        }
        ...
        public void ApplyImpulse(Vector3 impulse)
        {
            ...
            if (Object.HasStateAuthority)
            {
                ...
                _cc.Move(Vector3
                    .zero); // Velocity property is only used by CC when steering, so pretend we are, without actually steering anywhere
            }
        }
        ...
        public void ApplyDamage(Vector3 impulse, byte damage, PlayerRef attacker)
        {
            ...
            if (attackingPlayer == null ||
                attackingPlayer.playerID == playerID && !CanApplyDamage(attackingPlayer.battleID))
                return;
        ...
        private bool CanApplyDamage(BattleID attackerFaction)
        {
            ...
            if ((attackerFaction == BattleID.ROCK && battleID == BattleID.SCISSORS) ||
                (attackerFaction == BattleID.SCISSORS && battleID == BattleID.PAPER) ||
                (attackerFaction == BattleID.PAPER && battleID == BattleID.ROCK))
            {
                return true;
            }
            else
        ...
        private void CheckRespawn()
        {
            ...
            {
                Debug.Log(
                    $"Respawning player {playerID}, life={life}, lives={lives}, battleID = {battleID}, hasAuthority={Object.HasStateAuthority} from state={state}");
                ...
                Debug.Log(
                    $"Respawned player {playerID}, tick={Runner.Simulation.Tick}, timer={respawnTimer.IsRunning}:{respawnTimer.TargetTick}, life={life}, lives={lives},  battleID = {battleID}, hasAuthority={Object.HasStateAuthority} to state={state}");
            }
        }
        ...
        {
            Debug.Log(
                $"Resetting player {playerID}, tick={Runner.Simulation.Tick}, timer={respawnTimer.IsRunning}:{respawnTimer.TargetTick}, life={life}, lives={lives}, hasAuthority={Object.HasStateAuthority} to state={state}");
        ...
        private void CheckForPowerupPickup()
        {
            ...
            if (isActivated && Runner.GetPhysicsScene().OverlapSphere(transform.position, _pickupRadius, _overlaps,
                _pickupMask, QueryTriggerInteraction.Collide) > 0)
            {
